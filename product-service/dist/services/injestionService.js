...
function mapWooToProductRow(p) {
    const category = Array.isArray(p.categories) && p.categories.length ? (p.categories[0].name ?? null) : null;
    const tags = Array.isArray(p.tags) ? p.tags.map((t) => t.name).filter(Boolean) : [];
